#include "cldl32.h"
__declspec(dllexport) int Font(HWND hWnd,struct TPCLIPINFO *ClipInfo,int Flag,int cnt)
{
	...
	Dlg(ret,GetForegroundWindow(),"size=?","Please input size","");
	if(strcmp(ret,"\0")!=0){
		wsprintf(buffer," size=\"%s\"",ret);
		strcat(options,buffer);
	}

	Dlg(ret,GetForegroundWindow(),"color=?","Please input color","");
	if(strcmp(ret,"\0")!=0){
		wsprintf(buffer," color=\"%s\"",ret);
		strcat(options,buffer);
	}
	wsprintf(ret,"<font%s>%s</font>",options,buf);
